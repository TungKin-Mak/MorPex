...
- **发现机制**：仓库根 `AGENTS.md` 是主流 Agent 工具（Cursor/Codex/Claude Code 等）自动发现的标准入口文件；本文件（CLAUDE.md）是 Claude 系铁律。两者都指向 `SESSION_LOG.md`。
- **会话开始时**：必须先读 `SESSION_LOG.md`（含项目概览、会话历史、当前待办、关键路径），禁止对项目一无所知地开始。若工具不自动读文件，请显式执行 `cat SESSION_LOG.md AGENTS.md CLAUDE.md` 后再动手。
