...
async function main() {
  ...
  const { ServiceContainer } = await import('../../packages/core/src/execution/runtime/ServiceContainer.js');
  ...
  let t = Date.now();
  try {
    const { GoalIntelligenceFacade } = await import('../../packages/core/src/planner/goal-intelligence/GoalIntelligenceFacade.js');
