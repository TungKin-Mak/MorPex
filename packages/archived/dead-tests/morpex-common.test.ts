...
{
  try {
    const m = await import('../src/infrastructure/common/ModelRegistry.js');
...
{
  try {
    const t = await import('../src/infrastructure/common/ThinkingLevelControl.js');
...
console.log('\n📋 4. extractJson\n');
{
  const { extractJson } = await import('../src/infrastructure/utils/extractJson.js');
...
console.log('\n📋 5. toposort\n');
{
  const { topologicalSort } = await import('../src/infrastructure/utils/toposort.js');
  const items = [
    { id: 'a', deps: [] as string[] },
    { id: 'b', deps: ['a'] },
    { id: 'c', deps: ['a', 'b'] },
...
console.log('\n📋 6. jsonl\n');
{
  const { readJSONLLines } = await import('../src/infrastructure/utils/jsonl.js');
  const content = [
    '{"a":1,"b":"hello"}',
    '{"a":2,"b":"world"}',
    'invalid json line',
...
{
  try {
    const { AsyncResourceLocker, VersionConflictError } = await import('../src/infrastructure/utils/AsyncResourceLocker.js');
