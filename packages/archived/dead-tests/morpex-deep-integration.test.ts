...
{
  const { PluginSystem } = await import('../src/infrastructure/common/PluginSystem.js');
...
{
  const memHooks = await import('../src/knowledge/memory/MemoryHooks.js');
...

{
  ...
  const mmOk = await import('../src/knowledge/memory/MemoryMessages.js').then(() => true).catch(() => false);
...
{
  const { createKnowledgeGraphSkill } = await import('../src/infrastructure/tools/knowledge-graph-skill.js');
...
{
  const { createArtifactRegistrySkill } = await import('../src/infrastructure/tools/artifact-registry-skill.js');
