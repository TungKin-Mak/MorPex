 ...
 *
 * 用法：
 *   - `scrubEnv(env)` 于任何 spawn/exec 前调用。
 *   - `runCommand()` 返回 `ExecOutcome`——四因子各自独立字段，消费端按需读取。
 *   - `makePrivateTempDir()` / `randomPrivateFilePath()` / `writeExclusive()` 供临时落盘。
 *
...
export function runCommand(opts: RunCommandOptions): Promise<ExecOutcome> {
  ...
  return new Promise<ExecOutcome>((resolve) => {
    ...
    if (timeoutMs && timeoutMs > 0) {
      timer = setTimeout(() => {
        timedOut = true;
        // 先温和后强制：SIGTERM 后若进程未退，close 事件会带 signal 结算；
