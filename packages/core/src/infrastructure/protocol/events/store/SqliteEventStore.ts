...
import { CompactionService } from '../../../../infrastructure/observability/CompactionService.js';
import type { CompactionConfig } from '../../../../infrastructure/observability/CompactionService.js';
...
export class SqliteEventStore implements IEventStore {
  ...
  enableAutoCompaction(intervalMs: number = 12 * 60 * 60 * 1000): void {
    ...
    this.compactionTimer = setInterval(async () => {
      try {
        const { CompactionService } = await import('../../../../infrastructure/observability/CompactionService.js')
