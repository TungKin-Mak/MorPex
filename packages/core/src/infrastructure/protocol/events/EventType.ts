 ...
 *
 * ⚠️ 双轨说明（2026-08-03 对账）：
 *   本枚举**非死代码**——被 EVENT_LAYERS（事件分层索引）消费（102/105 成员被引用）。
 *   但运行时真实 emit 多经字符串字面量直接发出（如 gate.rule.* / ontology.* / context.* / execution.*），
 *   与本枚举互不相通（双轨）：枚举≠事件全集。
 *   事件归一化（枚举对齐字符串）为独立 Wave，不在本次范围。
 */
