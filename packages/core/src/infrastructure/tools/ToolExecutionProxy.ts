...
import type { AgentToolResult as _AgentToolResult } from '../../infrastructure/adapters/pi-types.js';
...
export class ToolExecutionProxy {
  ...
  async execute(
    ...
    // Note: Memory monitoring uses process.memoryUsage() polling rather than
    // worker.resourceLimits events because resourceLimits events aren't reliably
    // emitted in all Node.js versions. Polling at 5s intervals is sufficient for
    // OOM prevention without significant overhead.
    // 3. 内存监控（每 5 秒采样）
    ...
    return new Promise<Record<string, unknown>>((resolve, reject) => {
      ...
      if (!retried) {
        retried = true;
        console.warn(`[ToolProxy] Worker ${toolCallId} 失败(${(err as Error).message})，尝试降级重试...`);
        return this.execute(toolCallId, toolName, args, workingDir, onProgress);
      }
      // 6. 向 FSMEngine 抛出标准异常
      throw new ToolExecutionTimeoutError(toolCallId, toolName, (err as Error).message);
    });
  }
