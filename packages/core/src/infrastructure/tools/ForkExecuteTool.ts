...
import type { AgentTool, AgentToolResult as _AgentToolResult } from '../../infrastructure/adapters/pi-types.js';
type AgentToolResultAny = _AgentToolResult;
import { Type, optionalProp } from '../../infrastructure/adapters/pi-ai-types.js';
import { ToolExecutionProxy } from './ToolExecutionProxy.js'; // tools/ sibling
...
export class ForkExecuteTool implements AgentTool {
  ...
  parameters = Type.Object({
    ...
    timeout_ms: optionalProp(Type.Number({ description: '超时毫秒（默认 120000）' })),
  });
  ...
  async execute(
    ...
    if (script_type === 'bash') {
      result = await this.proxy.execute(
        ...
        { command: payload },
        process.cwd(),
        onUpdate ? (partial: Record<string, unknown>) => onUpdate(partial) : undefined,
      );
    ...
    // 归一化结果：附加执行摘要
    const c = result as unknown as { content?: Array<{ type: string; text?: string }> };
