...
```bash
npm run test:full        # 一键全部（tsc/架构/vitest 790 用例/生产/CLI）
...
**测试**：**790 用例 / 91 文件**（core 775 + e2e + G1 事件契约 7 + G3 安全路径 3，含去黑盒化 16n +5 用例），覆盖矩阵 8 层 ❌ 清零；一键 `npm run test:full` 全绿；覆盖率行覆盖 37%+（vitest 阈值防回退）。详见 `docs/TESTING_PLAN.md`。
...
| **测试用例** | **790**（vitest 91 文件，覆盖矩阵 ❌ 清零；含去黑盒化 16n） |
