...
| `knowledge/context/retrieval/ContextRetriever.ts` | ContextRetriever — 上下文相关性检索器（v16i RAG-lazy 装配）：按 goal 语义/domain 打分检索 Top-K 任务上下文+经验+策略 → 指针+蒸馏摘要（替代最近 N 全量注入，省 token 保质量） | 权威存储+Tier写规则；只检索不生成 |
