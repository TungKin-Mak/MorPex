...
| `execution/runtime/StepEventRecorder.ts` | StepEventRecorder — 订阅总线 workflow.step_* 五类事件落入 PersistentMissionStore（EventBus Only 合规；missionId 归属过滤；attach 幂等守卫） | 只订阅不发射；不直连 DAGRuntime |
