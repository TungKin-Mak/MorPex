...
| `governance/control-plane/SpaceService.ts` | SpaceService — 组织空间服务（扫描 WorkflowProvider → 构建部门 Space + 别名映射 + 持久化 + routeGoal 兜底）；提示词已收编至 governance/prompts/space-prompts.ts（逐字等价） | 目标级授权；不推理/不执行/不直接查知识 |
| `governance/prompts/space-prompts.ts` | 组织空间提示词资产 — buildManagerPersona/buildRouteHint（P1 #3 收尾批自 governance/control-plane/SpaceService.ts 内联模板逐字迁入，只改引用不改文案；零依赖可 tree-shaking） | 资产化存储；调优改此文件 |
...
| `cognition/prompts/delivery-prompts.ts` | 快捷分解提示词资产 — buildQuickDecomposePrompt（P1 #3 第四批自 DeliveryPlanner 内联逐字迁入，只改引用不改文案；零依赖可 tree-shaking） | 资产化存储；调优改此文件 |
| `cognition/prompts/company-prompts.ts` | 闲聊直答提示词资产 — CHAT_REPLY_SYSTEM（P1 #3 收尾批自 facade/CompanyFacade.ts 内联常量逐字迁入，只改引用不改文案；零依赖可 tree-shaking） | 资产化存储；调优改此文件 |
...
| `knowledge/prompts/bootstrap-prompts.ts` | bootstrap 提示词资产 — buildBootstrapSystemPrompt/buildBootstrapUserPrompt（P1 #3 收尾批自 knowledge/ontology/bootstrapFromDocs.ts 内联模板逐字迁入，只改引用不改文案；零依赖可 tree-shaking） | 资产化存储；调优改此文件 |
...
| `cognition/planning/DeliveryPlanner.ts` | DeliveryPlanner — 统一规划引擎（Facade） Phase 2 / 交付层 对外提供统一的规划入口，对内委托给: - MetaPlanner（完整 7 引擎规划管线） - SimulationEngine（执行前仿真预测） 设计原则： - Facade 模式：不修改现有模块 - 根据任务复杂度自动选择规划路径 - 支持 "快速规划"（简单任务跳过仿真） - 支持 "完整规划"（复杂任务走全管线） 规划模式： - 'quick': 快速；提示词已收编至 prompts/delivery-prompts.ts（逐字等价） | 理解/推理/规划；禁副作用Primitive/不改知识/不触发演化 |
...
| `execution/AgentMailbox.ts` | AgentMailbox — 跨部门/工位真交流（P2）：LLM 扮演目标角色生成回复 + 落盘 data/mailbox/*.jsonl + 事件 agent.message；提示词已收编至 prompts/mailbox-prompts.ts（逐字等价） | 有界执行+硬边界；不重规划/不评分/不演化 |
...
| `studio/server/hook-hardening.ts` | hook-hardening — /api/hooks/trigger 公网加固三件套：HookDedup(event-id 去重 JSON 原子写+7天TTL)、createFixedWindowLimiter(固定窗口限流 env 可配)、GOAL_MAX_CHARS(413 上限)；定位=安全兜底非触发机制 | studio 侧；不含触发判断逻辑 |
| `execution/runtime/RunRegistry.ts` | RunRegistry — 运行控制注册表（missionId→paused/cancelled 标志）；DAGRuntime 每轮迭代经 shouldPause/shouldCancel 钩子读取；P0-2 起 hydrate(id,state) 支持从事件源重放水合（ServiceContainer init 后调用），pause 态重启不丢失 | core 侧内存标志 + 水合入口；IO 由 PersistentMissionStore 事件源承担 |
...
| `execution/prompts/mailbox-prompts.ts` | Mailbox 角色扮演提示词资产 — buildMailboxSystemPrompt/buildMailboxUserPrompt/MAIL_FALLBACK_REPLY（P1 #3 第四批自 AgentMailbox 内联逐字迁入，只改引用不改文案；零依赖可 tree-shaking） | 资产化存储；调优改此文件 |
...
| `infrastructure/tools/paramCompleter.ts` | paramCompleter — 原语参数补全/校验层（inputSchema required 校验 + 缺失二次提取补全）；提示词已收编至 infrastructure/prompts/param-prompts.ts（逐字等价） | 底座服务；无领域逻辑/不推理/不规划/不评价/不演化 |
| `infrastructure/prompts/param-prompts.ts` | 参数补全提示词资产 — buildExtractPrompt（P1 #3 收尾批自 infrastructure/tools/paramCompleter.ts 内联模板逐字迁入，只改引用不改文案；零依赖可 tree-shaking） | 资产化存储；调优改此文件 |
