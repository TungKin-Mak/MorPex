...
5. **时效标记（防 LLM 误读过时）**：核心文档应感知时效（最后校验日期/版本）；引用已删模块、旧端口/结构、旧命令的文档 → **立即归档或加 `⚠️ 已过时（勿采信）` 横幅**，禁止留作“看似有效”。LLM 读到 ⚠️ 应忽略并提示清理。
6. **LLM 读取前甄别**：文档描述与代码不符（旧模块/旧端口/旧命令）→ 以代码为准（运行时>调用>测试>文档），并修文档或归档——防止基于过时文档做错误判断。
