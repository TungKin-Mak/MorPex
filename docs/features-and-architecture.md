...
MorPex v11 支持热插拔工作流插件，位于 `packages/workflows/`。

### 已注册插件

| 插件 | 路径 | 功能 |
|------|------|------|
| **xjmcu** | `workflows/xjmcu/` | 矽杰微 MCU 固件开发：代码生成→编译→烧录→仿真 |

插件通过 `manifest.json` 注册，引擎自动发现。每个插件持有工具链+知识源，但**共享主系统的 memory.db**。
